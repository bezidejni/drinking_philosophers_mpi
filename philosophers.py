...
class Philosopher(object):
    def __init__(self, index):
        ...
        self.listeners = {
            'L': comm.irecv(dest=self.neighbours['L']),
            'R': comm.irecv(dest=self.neighbours['R'])
        }
        self.requests = set()
    ...
    def sleep(self):
        print "  " * self.index + "{0}: sleeping".format(self.index)
        for i in range(random.randint(10, 40)):
            time.sleep(0.1)
            self.check_requests()

    def eat(self):
        print "  " * self.index + "{0}: I'm starting to eat".format(self.index)
        time.sleep(random.uniform(1, 2))
        self.hands['L'] = "D"
        self.hands['R'] = "D"
        print "  " * self.index + "{0}: I ate".format(self.index)
        ...
        while not self.has_both_forks():
            for side, value in self.hands.items():
                if not value and delay > 5:
                    comm.isend("gimme", dest=self.neighbours[side])
                    print "  " * self.index + "{0}: I'm asking for fork {1}".format(
                        self.index, self.neighbours[side])
                    delay = 0
            self.check_requests()
